...
//            let context = RecordListStore.Context(year: yearMonth.0, month: yearMonth.1)
//            self.recordListState = RecordListStore.State(context: context)
//            self.recordWritingState = RecordWritingStore.State(type: .good)
//            self.bottleListState = BottleListStore.State(context: RecordCountSummary(year: 2025, monthlyRecords: [:]))
...
//                let today = DateManager.shared.getFormattedDate(for: .today).components(separatedBy: "-")
//                var mainState = MainStore.State(today: today)

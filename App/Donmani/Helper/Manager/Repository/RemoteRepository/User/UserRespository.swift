...
final actor UserRespository {
    ...
    public func postRegisterUser(userKey: String) async throws -> User {
        let response = try await service.postRegisterUser(userKey: userKey)
        return response.toDomain()
    }
    
    /// 사용자 정보 업데이트
    public func postUpdateUser(userKey: String, newUserName: String) async throws -> User {
        let response = try await service.postUpdateUser(userKey: userKey, newUserName: newUserName)
        return response.toDomain()
    }
    ...
    public func putLastLogin(userKey: String) async throws {
        try await service.putLastLogin(userKey: userKey)
    }
    
    /// 공지사항 확인 상태 업데이트
    public func putNoticeStatus(userKey: String) async throws {
        try await service.putNoticeStatus(userKey: userKey)
    }
    
    /// 공지사항 확인 상태 조회
    func getNoticeStatus(userKey: String) async throws -> Bool {
        let response = try await service.getNoticeStatus(userKey: userKey)
    ...
    func putRewardStatus(userKey: String) async throws {
        try await service.putRewardStatus(userKey: userKey)
    }
    
    /// 새 리워드 아이템 확인 상태 조회
    func getRewardStatus(userKey: String) async throws -> Bool {
        let response = try await service.getRewardStatus(userKey: userKey)

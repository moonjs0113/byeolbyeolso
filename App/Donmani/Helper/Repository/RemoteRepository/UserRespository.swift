...
final actor UserRespository {
    ...
    public func postRegisterUser(userKey: String) async throws -> User {
        let response = try await self.dataSource.postRegisterUser(userKey: userKey)
        return response.toDomain()
    }
    
    /// 사용자 정보 업데이트
    public func postUpdateUser(userKey: String, newUserName: String) async throws -> User {
        let response = try await self.dataSource.postUpdateUser(userKey: userKey, newUserName: newUserName)
        return response.toDomain()
    }
    
    /// 마지막 로그인 업데이트
    public func putLastLogin(userKey: String) async throws {
        try await self.dataSource.putLastLogin(userKey: userKey)
    }
    
    /// 공지사항 확인 상태 업데이트
    public func putNoticeStatus(userKey: String) async throws {
        try await self.dataSource.putNoticeStatus(userKey: userKey)
    }
    
    /// 공지사항 확인 상태 조회
    func getNoticeStatus(userKey: String) async throws -> Bool {
        let response = try await self.dataSource.getNoticeStatus(userKey: userKey)
    ...
    func putRewardStatus(userKey: String) async throws {
        try await self.dataSource.putRewardStatus(userKey: userKey)
    }
    
    /// 새 리워드 아이템 확인 상태 조회
    func getRewardStatus(userKey: String) async throws -> Bool {
        let response = try await self.dataSource.getRewardStatus(userKey: userKey)
